...
import iot.client.extras.ASCIIArtGenerator;
...
public class CloudApp {
	...
	private SmartTemperature smart_temperature;
	private Thread smart_temperature_t;
	private CommandParser comm_parser;

	/* Constructor */
	public CloudApp() {

		res_dir = ResourceDirectory.getResourceDirectoryInstance();
		res_dir_t = new Thread(res_dir);
		res_dir_t.start();

		smart_lights = SmartLights.getSmartLightsInstance(res_dir);
		smart_lights_t = new Thread(smart_lights);
		smart_lights_t.start();

		smart_temperature = SmartTemperature.getSmartTemperatureInstance(res_dir);
		smart_temperature_t = new Thread(smart_temperature);
		smart_temperature_t.start();
